...
laduj<-reactive({
  ...
  output$plot <- renderPlot({
    ...
    if (input$freeze=="Go"){
      ...
      melted_A_e$type=paste('european ',(if (call){"call"}else{"put"}))
      ...
      rm(A,melted_A_before,melted_A_after)
      melted_A_a$type=paste('american ',(if (call){"call"}else{"put"}))
      ...
      ggplot(data=melted_A,aes(x=S,y=value,color=type))+
        ...
        labs(x="Underlying price (PLN)",y="Option price (PLN)",color="Option type:")+
        ...
        ggtitle(paste("Values of knock-and-out ",if (call){"call"}else{"put"}," with K=",sprintf("%.2f",input$strike),"PLN,B=",sprintf("%.2f",input$barrier),"PLN and d=",input$div,(if (divtype){"%"}else{"PLN"})," at t=",round(moment,3)))
    })
})
